...
"PIPELINE_ABLATION25 started $started" | Tee-Object -FilePath "logs\pipeline_ablation25.status.txt"

foreach ($variant in $variants) {
  foreach ($run in $runs) {
    $env:D5_WORKER_MODEL = $run.model
    $out = "logs\pipeline_ablation25_$($variant.label)_$($run.label).json"
    $console = "logs\pipeline_ablation25_$($variant.label)_$($run.label).console.log"
    $trace = "logs\llm_traces\pipeline_ablation25_$($runStamp)_$($variant.label)_$($run.label).jsonl"
    $env:D5_TRACE = "1"
    ...
    $line | Tee-Object -FilePath "logs\pipeline_ablation25.status.txt" -Append
    ...
    if ($code -ne 0) {
      $summary | ConvertTo-Json -Depth 4 | Set-Content -Encoding UTF8 "logs\pipeline_ablation25.summary.json"
      exit $code
    }
  }
}
...
$summary | ConvertTo-Json -Depth 4 | Set-Content -Encoding UTF8 "logs\pipeline_ablation25.summary.json"
"PIPELINE_ABLATION25 finished $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Tee-Object -FilePath "logs\pipeline_ablation25.status.txt" -Append
